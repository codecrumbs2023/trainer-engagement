...
const handleSubmit = async () => {
    try {
      const response = await axios.post("http://localhost:3001/purchase-orders", {
    ...
    return (
      <div className={`fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center ${isOpen ? "" : "hidden"}`}>
        <div className="bg-white p-4 rounded-lg w-96">
          ...
          <div className="mb-4">
            <p><strong>Batch Name:</strong> {request.batchName}</p>
            ...
            <p><strong>Training Budget:</strong> {request.trainingBudget}</p>
            <p><strong>Price Per Day:</strong> {calculatePricePerDay()}</p>
          </div>
